...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  // Generate ember-like colors
  const emberColor = (): string => {
    const colors = [
      "#ff4500", // orange red
      "#ff8c00", // dark orange
      "#ffa500", // orange
      "#ffd700", // gold
      "#ffffff", // occasional white spark
      "#d3d3d3", // gray ash
    ];
  ...
  const drawCircle = (circle: Circle, update = false) => {
    if (context.current) {
      ...
      context.current.fillStyle = color.startsWith("#")
        ? hexToRgbString(color, alpha)
  ...
  const hexToRgbString = (hex: string, alpha: number): string => {
    hex = hex.replace("#", "");
    if (hex.length === 3) {
      hex = hex
        .split("")
